    ...
    public class StopsController : Controller
    {
        ...
        public async Task<IActionResult> Post(string tripName, [FromBody]StopViewModel viewModel)
        {
            try
            {
                ...
                {
                    var newStopp = Mapper.Map<Stop>(viewModel);

                    //Lookup the Geocodes

                    //Save to the DB
                        _repository.AddStop(tripName, newStopp);

                        if (await _repository.SaveChangesAsync())
                        {
                            return Created($"/api/trips/{tripName}/stops/{newStopp.Name}",
                                Mapper.Map<StopViewModel>(newStopp));
                        }
                    }
            }
            catch(Exception ex)
